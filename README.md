...
This test evaluates the number of integer operations per second by performing encryption and decryption on a text file. Each character is turned into an integer, and multiple operations are performed for the encryption (subtraction, multiplication by 15435, bitwise OR operation with a 32 bit key, addition, and a division with a volatile burn variable which doesn't affect the process, but counts towards the operations). The results are written in an output file, which is used as input for decryption. The reverse of the process is performed for decryption, but instead of multiplying by 15435, the multiplication is done with its modular inverse. The same process applies here for running the benchmark, but lower and upper index represent the file size. All the files are generated by the program and deleted afterwards to keep the application compact. The results are the the execution time, millions of integer operations per second and the write time in the output files for each memory size.

### Floating Point Operations Tests

To measure the number of floating point operations (FLOPS) per second, the computation of the Mandelbrot Set has been used. The operations are counted, and with that, the average FLOPS/s are obtained. The final results are the average execution times and FLOPS/s (measured in MFLOPS). The process of running the test is the same as the others, except this one does not have an option to change the chart type (just like in the Mmeory Transfer Tests).

### CPU Grader

This function performs all of the aforementioned tests, with a specific and unchangeable dataset. The user is informed beforehand of what tests will be ran. Once *Start Grader* button is clicked, the tests will commence, and a small progress bar will appear on the bottom of the window. At the end, all the graphs from each test are displayed, as well as the results of the most demanding test from each category. Alongside the results, a grade will be given. The charts can also be saved by pressing the *Save Charts* button. They are saved in the same directory as the executable, in a new folder called *charts*. As said before, this was mostly a tool to help check how the program runs on other people's computers without taking a large amount of individual screenshots.
